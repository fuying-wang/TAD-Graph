...
from tadgraph.utils.file_utils import load_pkl, save_pkl
from tadgraph.wsi_core.util_classes import (Contour_Checking_fn, isInContourV1,
                                       isInContourV2, isInContourV3_Easy,
                                       isInContourV3_Hard)
from tadgraph.wsi_core.wsi_utils import (coord_generator, initialize_hdf5_bag,
...
#         self.name = ".".join(path.split("/")[-1].split('.')[:-1])
        ...
        # self.wsi.properties[openslide.PROPERTY_NAME_MPP_X]
    ...
    def initXML(self, xml_path):
        ...
        annotations = [anno.getElementsByTagName(
            'Coordinate') for anno in xmldoc.getElementsByTagName('Annotation')]
        self.contours_tumor = [_createContour(
            coord_list) for coord_list in annotations]
            ...
            for idx, annot_group in enumerate(annot):
                ...
                if annot_group['type'] == 'Polygon':
                    for idx, contour in enumerate(contour_group):
                        contour = np.array(contour).astype(
    ...
    def saveSegmentation(self, mask_file):
        ...
        asset_dict = {'holes': self.holes_tissue,
    ...
    def segmentTissue(self, seg_level=0, sthresh=20, sthresh_up=255, mthresh=7, close=0, use_otsu=False,
        ...
        def _filter_contours(contours, hierarchy, filter_params):
            ...
            for cont_idx in hierarchy_1:
                ...
                hole_areas = [cv2.contourArea(
                    contours[hole_idx]) for hole_idx in holes]
        ...
        img = np.array(self.wsi.read_region(
            (0, 0), seg_level, self.level_dim[seg_level]))
        ...
        if len(keep_ids) > 0:
            ...
            contour_ids = set(
                np.arange(len(self.contours_tissue))) - set(exclude_ids)
    ...
    def visWSI(self, vis_level=0, color=(0, 255, 0), hole_color=(0, 0, 255), annot_color=(255, 0, 0),
        ...
        if not view_slide_only:
            ...
            line_thickness = int(
                line_thickness * math.sqrt(scale[0] * scale[1]))
    ...
    def createPatches_bag_hdf5(self, save_path, patch_level=0, patch_size=256, step_size=256, save_coord=True, **kwargs):
        ...
        for idx, cont in enumerate(contours):
            ...
            if self.hdf5_file is None:
                ...
                file_path = initialize_hdf5_bag(
                    first_patch, save_coord=save_coord)
    ...
    def _getPatchGenerator(self, cont, cont_idx, patch_level, save_path, patch_size=256, step_size=256, custom_downsample=1,
        ...
        patch_downsample = (int(self.level_downsamples[patch_level][0]), int(
            self.level_downsamples[patch_level][1]))
        ref_patch_size = (
        ...
        for y in range(start_y, stop_y, step_size_y):
            for x in range(start_x, stop_x, step_size_x):
                ...
                patch_PIL = self.wsi.read_region(
                    (x, y), patch_level, (patch_size, patch_size)).convert('RGB')
                if custom_downsample > 1:
    ...
    def process_contour(self, cont, contour_holes, patch_level, save_path, patch_size=256, step_size=256,
        ...
        start_x, start_y, w, h = cv2.boundingRect(cont) if cont is not None else (
            0, 0, self.level_dim[0], self.level_dim[1])

        # patch_downsample = (int(self.level_downsamples[patch_level][0]), int(self.level_downsamples[patch_level][1]))
        patch_downsample = (2 ** patch_level, 2 ** patch_level)
        ref_patch_size = (
            patch_size*patch_downsample[0], patch_size*patch_downsample[1])
        ...
        results = pool.starmap(
            WholeSlideImage.process_coord_candidate, iterable)
        pool.close()
        results = np.array(
        ...
        if len(results) > 1:
            ...
            downsampled_level_dim = (
                img_w // patch_downsample[0], img_h // patch_downsample[1])
    ...
    def visHeatmap(self, scores, coords, vis_level=-1,
        ...
        if binarize:
            overlay[~zero_mask] = np.around(
                overlay[~zero_mask] / counter[~zero_mask])
        else:
            overlay[~zero_mask] = overlay[~zero_mask] / counter[~zero_mask]
        del counter
        if blur:
            overlay = cv2.GaussianBlur(overlay, tuple(
                (patch_size * (1-overlap)).astype(int) * 2 + 1), 0)
        ...
        if not blank_canvas:
            ...
            # use blank canvas
            img = np.array(Image.new(size=region_size,
                           mode="RGB", color=(255, 255, 255)))
        ...
        for idx in range(len(coords)):
            ...
            if score >= threshold:
                ...
                # attention block
                raw_block = overlay[coord[1]:coord[1] +
                                    patch_size[1], coord[0]:coord[0]+patch_size[0]]
                ...
                color_block = (cmap(raw_block) *
                               255)[:, :, :3].astype(np.uint8)

                if segment:
                    # tissue mask block
                    mask_block = tissue_mask[coord[1]:coord[1] +
                                             patch_size[1], coord[0]:coord[0]+patch_size[0]]
                ...
                # rewrite image block
                img[coord[1]:coord[1]+patch_size[1], coord[0]
                    :coord[0]+patch_size[0]] = img_block.copy()
        ...
        if blur:
            img = cv2.GaussianBlur(img, tuple(
                (patch_size * (1-overlap)).astype(int) * 2 + 1), 0)

        if alpha < 1.0:
            img = self.block_blending(img, vis_level, top_left, bot_right,
                                      alpha=alpha, blank_canvas=blank_canvas, block_size=1024)
        ...
        for x_start in range(top_left[0], bot_right[0], block_size_x * int(downsample[0])):
            for y_start in range(top_left[1], bot_right[1], block_size_y * int(downsample[1])):
                ...
                if not blank_canvas:
                    ...
                    pt = (x_start, y_start)
                    canvas = np.array(self.wsi.read_region(
                        pt, vis_level, blend_block_size).convert("RGB"))
                else:
    ...
    def get_seg_mask(self, region_size, scale, use_holes=False, offset=(0, 0)):
        ...
        for idx in range(len(contours_tissue)):
            cv2.drawContours(image=tissue_mask, contours=contours_tissue,
                             contourIdx=idx, color=(1), offset=offset, thickness=-1)
